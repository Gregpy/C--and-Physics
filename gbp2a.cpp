 ...
 */
	...
	std::cout << "Using the 4th-order Runge-Kutta (RK4) method to solve the simple harmonic oscillator.\n";

	std::cout << "Enter initial condition 'x0':\n";
    std::getline(std::cin, in_val);

    x0 = std::stod(in_val);
	std::cout << "Enter initial condition 'v0':\n";
    std::getline(std::cin, in_val);

    v0 = std::stod(in_val);


	std::cout << "Enter the maximum time 'tmax':" << std::endl;

    std::getline(std::cin, in_val);

    tmax = std::stod(in_val);
	std::cout << "Enter the time steps 'M':" << std::endl;

    std::getline(std::cin, in_val);

    nsteps = std::stoi(in_val);
 ...
 */
	...
	std::cout << "Global position error = "<< x_err << std::endl;

	std::cout << "Data for 4th-order Runge-Kutta calculated position 'xRK', exact position 'xExact', 4th-order Runge-Kutta calculated velocity 'vRK', and exact velocity 'vExact' versus time 't' is sent to the file gbp2a.txt.\n";
