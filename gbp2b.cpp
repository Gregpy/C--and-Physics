...
main(int argc, char *argv[]) // main() with arguments!
{
	...
	std::cout << "Using the 4th-order Runge-Kutta (RK4) method with the fox-rabbit system of equations.\n";

	std::cout << "Enter initial number of rabbits 'nr0' :\n";
    std::getline(std::cin, in_val);

    nr0 = std::stod(in_val);

	std::cout << "Enter initial number of foxes 'nf0':\n";
    std::getline(std::cin, in_val);

    nf0 = std::stod(in_val);

	std::cout << "Enter the ratio of the starvation rate of foxes to the growth rate of rabbits rR/rF 'rho':\n";
    std::getline(std::cin, in_val);

    rho = std::stod(in_val);

	std::cout << "Enter the maximum time 'tmax':\n";
    std::getline(std::cin, in_val);

    tmax = std::stod(in_val);

	std::cout << "Enter the total number of time steps 'M':\n";
    std::getline(std::cin, in_val);
 ...
 */
	...
	std::cout << "Rabbits and foxes data 'nR(t)' and 'nF(t)' versus time 't' is sent to the file gbp2b.txt\n";
